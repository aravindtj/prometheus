...
const (
	...
	appendWorkers  = 8 // Should be enough to not make appending samples a bottleneck.
...
func (s *memorySeriesStorage) AppendSamples(samples clientmodel.Samples) {
	for _, sample := range samples {
		if s.getPersistQueueLen() >= s.persistQueueCap {
			glog.Warningf("%d chunks waiting for persistence, sample ingestion suspended.")
...
func (s *memorySeriesStorage) appendSample(sample *clientmodel.Sample) {
	...
	completedChunksCount := series.add(fp, &metric.SamplePair{
		Value:     sample.Value,
